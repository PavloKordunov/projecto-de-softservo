...
const SearchBar = () => {
    ...
    const backgroundColor = theme === 'dark' ? '#2C353D' : '#B5B5B5';
    ...
    return (
        <div ref={searchBarRef} className="relative w-[340px]  md:w-[300px] lg:w-[480px] 2xl:w-[580px]">
            ...
            <motion.input
                ...
                value={searchInfo}
                className={`relative z-20  w-full h-14 px-4 cursor-pointer py-2  ${theme === 'dark' ? 'bg-SecondaryColor placeholder:text-gray-300' : 'bg-[#B5B5B5] placeholder:text-gray-900'} border-b-[3px] ${theme === 'dark' ? 'border-SecondaryColor' : 'border-[#B5B5B5]'} rounded-[10px] focus:outline-none transition-all duration-300`}
                placeholder="Введіть для пошуку..."
                onFocus={handleFocus}
                onBlur={handleBlur}
                animate={{
                    width: isFocused ? "100%" : "80%",
                    backgroundColor: isFocused ? backgroundColor : theme === 'dark' ? '#2C353D' : '#B5B5B5',
                }}
                transition={{ duration: 0.3, ease: "easeOut" }}
            />

            {showSearchInfo && (
                <div className={`absolute w-[750px] cursor-pointer z-20 ${theme === 'dark' ? 'bg-SecondaryColor' : 'bg-[#B5B5B5]'} px-6 py-2 top-[80px] right-[50%] translate-x-1/2 flex flex-col gap-3 rounded-[11px]`}>
